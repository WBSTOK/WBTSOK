...
/** * Creates a shipping label through the Vercel API * @param {Object} customerData - Customer and order information * @returns {Promise<Object>} - API response */
async function createShippingLabel(customerData) {
  try {
    console.log('Sending shipping request:', customerData);
    
    // Use your Vercel serverless function instead of Heroku
    const response = await fetch('/api/proxy', {
      ...
      body: JSON.stringify({
        ...customerData,
        formType: 'shipping' // Add this to distinguish from contact forms
      })
    });
...
/** * Handles the checkout form submission * @param {Event} event - Form submission event */
async function handleCheckoutSubmit(event) {
  ...
  try {
    // Call the shipping API through Vercel
